...
(function ($) {
    ...
    SmartDataGrid.DataStore = function (source, clientCache) {
    ...
    function getColumnSortComparator(columnIndex, dataStoreColumns) {
        ...
        if (cellRenderer) {
            cellRenderer = $.smartDataGrid.renderer[cellRenderer];
        }
        ...
        if (filterColumns) {
            filterColumnsIndex = $.map(filterColumns, function (columnId) {
                ...
                columns[columnInfo.index] = {
                    column: column,
                    filterFunc: ($.smartDataGrid.filter[column.filterType] && $.smartDataGrid.filter[column.filterType].filter) || defaultFilter
                };
...
(function ($) {
    ...
    $.smartDataGrid = $.extend(true, $.smartDataGrid, {
        plugin: {
            groups: function ($smartDataGrid, options) {
                options = $.extend({
                    id: options.id,
                    isGridGroupable: true,
                    onGroupChange: $.noop,
                ...
                function onColumnGroupingDrop(event, ui) {
                    var columnId = $.smartDataGrid.getElementId(ui.draggable.attr("id"));
                ...
                function onGroupExpandCollapse(event) {
                    ...
                    setTimeout(function () {
                        ...
                        $smartDataGrid.find(".group-data").css({
                            zoom: 1
                        }).css({
                            zoom: 0
                        });
                ...
                function onGroupReorderDrop(event, ui) {
                    ...
                    var groupIdToMove = $.smartDataGrid.getElementId(ui.draggable.attr("id")),
                        groupIdToMoveAfter = $.smartDataGrid.getElementId($ele.attr("id")),
                        newGroupOrder = [];
                ...
                function load(helper) {
                    ...
                    $smartDataGrid.undelegate(".group .group-header", "click").delegate(".group .group-header", "click", onGroupExpandCollapse);
                    $groupsMainContainer = $(options.groupsPlaceHolder).undelegate("a.remove", "click.groups").delegate("a.remove", "click.groups", function () {
                        removeColumnFromGroup($.smartDataGrid.getElementId($(this).parents(".cell").eq(0).attr("id")));
                        return false;
                    });
...
(function ($) {
    ...
    $.smartDataGrid.updateRow = "smartDataGrid-update-row";

    $.smartDataGrid = $.extend(true, $.smartDataGrid, {
        plugin: {
            realTimeData: function ($smartDataGrid, options, cachedGridData) {
                ...
                function load(helper) {
                    ...
                    $smartDataGrid.unbind($.smartDataGrid.updateRow).bind($.smartDataGrid.updateRow, onRowUpdate);

                }

                function destroy() {
                    $smartDataGrid.unbind($.smartDataGrid.updateRow);
                    options = null;
                }
...
(function ($) {
    ...
    $.smartDataGrid.filter.DATE = {
        ...
        init: function ($filter, onFilter, gridId, $smartDataGrid) {
            ...
            $smartDataGrid.find(".smartDataGrid-rows").bind("scroll.datefilter-" + $filter.attr("id"), function () {
        ...
        destroy: function ($filter, $smartDataGrid) {
            $smartDataGrid.find(".smartDataGrid-rows").unbind("scroll.datefilter-" + $filter.attr("id"));
...
(function ($) {
    ...
    $.smartDataGrid.filter.FREE_TEXT = {
        render: function (id) {
...
(function ($) {
    ...
    $.smartDataGrid.filter.SIMPLE_DATE = {
        render: function (id) {
            var text = "<input id='{id}' type='text' class='{filterIdentifier}' value=''/>";
            return text.supplant({
                id: id
...
(function ($) {
    ...
    $.smartDataGrid.renderer.SYSTEM_DATE = {
        cell: function (data) {
