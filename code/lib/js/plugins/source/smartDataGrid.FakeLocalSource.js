...
(function ($) {
    ...
    SmartDataGrid.fakeData = {};
    SmartDataGrid.FakeLocalSource = function (dataset) {
        ...
        function callAction(actionType, requestData, callback) {
            ...
            var fakeDataStoreRows = SmartDataGrid.DataStore.utils.getFilteredRows($.extend(true, [], dataset.rows), stateData.filterBy, fakeDataStoreColumns, dataset.filters, true);
            if (hasGroupByOrSortBy(stateData)) {
                SmartDataGrid.DataStore.utils.sortData(stateData, fakeDataStoreColumns, fakeDataStoreRows, true);
            }
            var dummyResponseData = {};
            var pageSize = (requestData.pageSize || dataset.pageSize || 25);
            if (actionType === "fetchRows") {
                dummyResponseData = {
                    rows: SmartDataGrid.DataStore.utils.getRows(stateData.pageOffset - 1, pageSize, fakeDataStoreRows)
                };
            } else {
        ...
        function getDummyDataForFirstRequest(pageSize, fakeDataStoreColumns, fakeDataStoreRows) {
            return {
                ...
                rows: SmartDataGrid.DataStore.utils.getRows(0, pageSize, fakeDataStoreRows),
                state: {
                    groupBy: [],
                    sortBy: []
                },
    ...
    SmartDataGrid.FakeLocalSource.getFilterData = function (columnIdentifier, rows) {
        var filterData = [];
